/* eslint-disable @typescript-eslint/no-unsafe-return, no-param-reassign, @typescript-eslint/restrict-plus-operands */
import { mergeMap } from "rxjs/operators";
import { Observable, of } from "rxjs";
import { TestScheduler } from "rxjs/testing";
import { isEqual } from "lodash-es";
import { KeyCompareMap, rxDistinctUntilKeysChange } from "./rx-distinct-until-keys-change";
...
describe('rxDistinctUntilKeysChange operator', () => {
    it('should distinguish between values', () => {
        testScheduler.run(({ cold, expectObservable, expectSubscriptions }) => {
            ...
            expectObservable(e1.pipe(rxDistinctUntilKeysChange(['val']))).toBe(expected, values);
    ...
    it('should distinguish between values with multiple keys', () => {
        testScheduler.run(({ cold, expectObservable, expectSubscriptions }) => {
            ...
            expectObservable(e1.pipe(rxDistinctUntilKeysChange(['val', 'valOther']))).toBe(expected, values);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    ...
    it('should distinguish between values by keyCompareMap', () => {
        testScheduler.run(({ cold, expectObservable, expectSubscriptions }) => {
            ...
            expectObservable(e1.pipe(rxDistinctUntilKeysChange(['val', 'objVal'], keyCompare))).toBe(expected, values);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    ...
    it('should not completes if source never completes', () => {
        testScheduler.run(({ cold, expectObservable, expectSubscriptions }) => {
            ...
            expectObservable((<Observable<any>>e1).pipe(rxDistinctUntilKeysChange(['val']))).toBe(expected);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    ...
    it('should allow unsubscribing early and explicitly', () => {
        testScheduler.run(({ cold, expectObservable, expectSubscriptions }) => {
            ...
            const result = e1.pipe(rxDistinctUntilKeysChange(['val']));
    ...
    it('should emit once if comparer returns true always regardless of source emits', () => {
        testScheduler.run(({ cold, expectObservable, expectSubscriptions }) => {
            ...
            expectObservable(e1.pipe(rxDistinctUntilKeysChange(['val'], { val: () => true }))).toBe(expected, values);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    ...
    it('should emit all if comparer returns false always regardless of source emits', () => {
        testScheduler.run(({ cold, expectObservable, expectSubscriptions }) => {
            ...
            expectObservable(e1.pipe(rxDistinctUntilKeysChange(['val'], { val: () => false }))).toBe(expected, values);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    ...
    it('should distinguish values by selector', () => {
        testScheduler.run(({ cold, expectObservable, expectSubscriptions }) => {
            ...
            expectObservable(e1.pipe(rxDistinctUntilKeysChange(['val'], { val: selector }))).toBe(expected, values);
            expectSubscriptions(e1.subscriptions).toBe(e1subs);
        });
    });
    ...
    it('should raises error when comparer throws', () => {
        testScheduler.run(({ cold, expectObservable, expectSubscriptions }) => {
            ...
            expectObservable(e1.pipe(rxDistinctUntilKeysChange(['val'], { val: selector }))).toBe(
                expected,
                values,
                new Error('error')
            );
    ...
    it('should handle the defined edge cases', () => {
        ...
        testScheduler.run(({ cold, expectObservable }) => {
            testSet.forEach(test => {
                ...
                */
                expectObservable(e1.pipe(rxDistinctUntilKeysChange(['str']))).toBe(expected, test.expectValues);
            });
        });
    });
});
