/* eslint-disable @typescript-eslint/no-unsafe-assignment, @typescript-eslint/no-explicit-any, @typescript-eslint/no-unsafe-argument, @typescript-eslint/no-unsafe-member-access, @typescript-eslint/no-unsafe-call, @typescript-eslint/naming-convention
 */
import { ErrorHandler } from "@angular/core";
import { merge, OperatorFunction, Subject } from "rxjs";
import { EffectMap, KeysOf, ReactiveActions, SubjectMap, ValuesOf } from "./types";
...
export function actionProxyHandler<T extends object, U extends object>({
...
}): ProxyHandler<ReactiveActions<T, U>> {
    ...
    function dispatch(value: ValuesOfT, prop: KeysOfT) {
        ...
        try {
            const val = transformsMap && (transformsMap as any)[prop] ? (transformsMap as any)[prop](value) : value;
    ...
    return {
        ...
        apply(_: ReactiveActions<T, U>, __: any, props: [T]): any {
            props.forEach(slice => Object.entries(slice).forEach(([k, v]) => dispatch(v, k as any as KeysOfT)));
        },
        get(_, property: string) {
            ...
            if (prop.toString().startsWith('on')) {
                ...
                return (behaviour: OperatorFunction<T[KeysOfT], T[KeysOfT]>, sf: (v: T[KeysOfT]) => void) => {
