...
export type ActionTransforms<T extends object> = Partial<{
    [K in keyof T]: (...args: Array<any>) => T[K];
}>;

export type ActionDispatchFn<O extends Array<unknown>> = (...value: InstanceOrType<O>) => void;
...
export type ReactiveActions<T extends Actions, U extends object = T> = ActionDispatchers<T, U> &
    ActionObservables<T> &
    ActionEffects<T> &
    ((slice: Partial<T>) => void) & {
        $: (props: Array<keyof T>) => Observable<ValuesOf<T>>;
    };
