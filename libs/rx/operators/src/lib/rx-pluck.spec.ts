...
describe('rxPluck', () => {
  ...
  it('should work for one array', () => {
    testScheduler.run(({ cold, expectObservable, expectSubscriptions }) => {
      ...
      const result = e1.pipe(rxPluck(0));
  ...
  it('should work for one object', () => {
    testScheduler.run(({ cold, expectObservable, expectSubscriptions }) => {
      ...
      const result = e1.pipe(rxPluck('prop'));
  ...
  it('should work with deep nested properties', () => {
    testScheduler.run(({ cold, expectObservable, expectSubscriptions }) => {
      ...
      const result = e1.pipe(rxPluck('a', 'b', 'c'));
  ...
  it('should work with edge cases of deep nested properties', () => {
    testScheduler.run(({ cold, expectObservable, expectSubscriptions }) => {
      ...
      const result = e1.pipe(rxPluck('i', 'j', 'k'));
  ...
  it('should throw an error if not property is passed', () => {
    expect(() => {
      of({ prop: 1 }, { prop: 2 }).pipe(rxPluck());
    }).toThrow( 'list of properties cannot be empty.');
  });
  ...
  it('should propagate errors from observable that emits only errors', () => {
    testScheduler.run(({ cold, expectObservable, expectSubscriptions }) => {
      ...
      const result = e1.pipe(rxPluck('whatever'));
  ...
  it('should pluck twice', () => {
    testScheduler.run(({ cold, expectObservable, expectSubscriptions }) => {
      ...
      const result = e1.pipe(rxPluck('a', 'b'), rxPluck('c'));
  ...
  it('should support symbols', () => {
    testScheduler.run(({ cold, expectObservable, expectSubscriptions }) => {
      ...
      const result = e1.pipe(rxPluck(sym));
  ...
  it('should stop listening to a synchronous observable when unsubscribed', () => {
    ...
    synchronousObservable.pipe(rxPluck('whatever'), take(3)).subscribe(() => {
      /* noop */
    });
