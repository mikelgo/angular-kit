import { valuesComparer } from './valuesComparer.util';
  ...
  describe('functionality', () => {
    describe('primitives', () => {
      ...
      it('should return true if matching by compareFn', () => {
        expect(valuesComparer(1, 1, (a: { toString: () => any; }, b: { toString: () => any; }) => a.toString() === b.toString()));
      });

      it('should return false if matching by compareFn', () => {
        expect(valuesComparer(1, 2, (a: { toString: () => any; }, b: { toString: () => any; }) => a.toString() === b.toString()));
      });
    });
    ...
    describe('non-primitives', () => {
      ...
      it('should return true if items match by CompareFn', () => {
        expect(valuesComparer(item, clone, (a: { id: any; }, b: { id: any; }) => a.id === b.id)).toBeTruthy();
      });

      it('should return false if items do nott match by CompareFn', () => {
        expect(valuesComparer(item, otherItem, (a: { id: any; }, b: { id: any; }) => a.id === b.id)).toBeFalsy();
      });
